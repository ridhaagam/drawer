...
import { hasStrokeColor, canChangeRoundness } from "@excalidraw/element";
...
export const actionChange3DDepth = register({
  name: "change3DDepth",
  label: "labels.3dDepth",
  trackEvent: false,
  perform: (elements, appState, value) => {
    return {
      elements: changeProperty(elements, appState, (el) => {
      ...
      appState: { ...appState, currentItem3DDepth: value },
      captureUpdate: CaptureUpdateAction.IMMEDIATELY,
    };
  },
  PanelComponent: ({ elements, appState, updateData, app, data }) => {
    ...
    return (
      <fieldset>
        <legend>{t("labels.3dDepth")}</legend>
        <div className="buttonList">
          <input
            type="range"
            min="-500"
...
export const actionChangeEdgeRoundness = register({
  name: "changeEdgeRoundness",
  label: "labels.edgeRoundness",
  trackEvent: false,
  perform: (elements, appState, value) => {
    return {
      elements: changeProperty(elements, appState, (el) => {
        if (!canChangeRoundness(el.type)) {
          return el;
        }
      ...
      appState: { ...appState, currentItemEdgeRoundness: value },
      captureUpdate: CaptureUpdateAction.IMMEDIATELY,
    };
  },
  PanelComponent: ({ elements, appState, updateData, app }) => {
    ...
    return (
      <fieldset>
        <legend>{t("labels.edgeRoundness")}</legend>
        <div className="buttonList">
          <input
            type="range"
            min="0"
