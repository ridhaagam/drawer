...
// Lazy load PresentationOverlay to break circular import
// (PresentationOverlay imports useExcalidrawActionManager from this file)
const PresentationOverlay = lazy(() => import("./PresentationOverlay"));
...
class App extends React.Component<AppProps, AppState> {
  ...
  private onKeyDown = withBatchedUpdates(
    (event: React.KeyboardEvent | KeyboardEvent) => {
      ...
      // Handle Ctrl+1 for cube and Ctrl+2 for rectangular prism
      if (
        event[KEYS.CTRL_OR_CMD] &&
        !event.altKey &&
        !event.shiftKey &&
        (event.key === KEYS["1"] || event.key === KEYS["2"])
      ) {
        const shape = event.key === KEYS["1"] ? "cube" : "rectangularPrism";
  ...
  private createGenericElementOnPointerDown = (
    ...
    if (elementType === "embeddable") {
      ...
      element = newCubeElement({
        ...baseElementAttributes,
      });
    } else if (elementType === "rectangularPrism") {
      element = newRectangularPrismElement({
        ...baseElementAttributes,
      });
    } else {
      element = newElement({
